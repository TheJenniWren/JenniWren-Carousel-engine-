...
'''
    ...
    old_import = '''document.getElementById("import-json-button").addEventListener("click",()=>{
...
'''
    new_import = '''document.getElementById("import-json-button").addEventListener("click",async()=>{
  const raw=document.getElementById("import-json").value.trim();
  if(!raw){showImportStatus("Paste a carousel JSON package first.","error");return}
  try{
    const data=JSON.parse(raw);
...
'''
    text = replace_once(text, old_import, new_import, "browser import validation")
