...
'''
    ...
    # Browser import block varies slightly between 3.8 builds. Replace it
    # structurally rather than requiring an exact full-string match.
    import_pattern = re.compile(
        r'document\\.getElementById\\("import-json-button"\\)'
  ...
  }catch(error){showImportStatus(`Import rejected: ${error.message}`,"error")}
});'''
    text, import_count = import_pattern.subn(new_import, text, count=1)
    if import_count != 1:
        raise PatchError(
            f"browser import validation: expected exactly one listener, found {import_count}."
        )
