...
BACKUP = ROOT / "carousel_lib.py.before_stat_grid_v3_8_27"

NEW_DRAW_STAT_GRID = '''def draw_stat_grid(draw, items, y0, cols=2, cell_h=285):
    """Stat Grid v3.8.27: move the entire grid visibly right and rebalance its center."""
    items = items or []
    if not items:
        return y0
    ...
    GRID_LEFT_SHIFT = 48
    GRID_RIGHT_SHIFT = 24
    GRID_LEFT = L_MARGIN + GRID_LEFT_SHIFT
    GRID_RIGHT = W - R_MARGIN + GRID_RIGHT_SHIFT
    ...
    for idx, (stat_text, label_text) in enumerate(normalized):
        ...
        cell_x = int(GRID_LEFT + col * col_w)
...
def main() -> int:
    ...
    print("Stat Grid v3.8.27 applied successfully.")
    print("Moved the entire grid right and rebalanced its visual center.")
