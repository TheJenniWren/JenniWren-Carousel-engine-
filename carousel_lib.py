...
v3.8.0
------
This revision integrates Headline Engine v2 while preserving the public
carousel_lib API used by all existing template modules. Existing calls to
draw_headline(draw, lines, colors, y0, sr) continue to work unchanged, but
headline measurement, fitting, per-line color rendering, optical placement,
and diagnostics are now delegated to the shared deterministic engine.
Additions in this revision are:

* Headline Engine v2 integration through the existing draw_headline() API.
* Manual line breaks preserved exactly; no automatic headline rewriting.
* Per-line white/pink colors, including dashboard string color names.
* Larger headline ranges supported up to 180px (or the caller-supplied maximum).
* Optical vertical balancing within the assigned headline region.
* Structured diagnostics exposed through get_last_headline_diagnostics().
...
All non-headline rendering behavior remains unchanged. Legacy fit_head() and
fit_head_custom() remain available for compatibility and specialized template
work, while draw_headline() now uses Headline Engine v2 as the production
headline path.
"""
...
# Headline Engine v2 is required for Studio 3.8 production rendering.
try:
    from .headline_engine_v2 import (
        HeadlineDiagnostics,
        diagnostic_note as _headline_diagnostic_note,
        render_headline as _render_headline_v2,
    )
...
# ── HEADLINE ENGINE V2 DIAGNOSTICS (v3.8) ───────────────────────────────
_LAST_HEADLINE_DIAGNOSTICS: Optional[HeadlineDiagnostics] = None
_HEADLINE_QA_NOTES: List[str] = []


def _resolve_headline_color(value: Any) -> Tuple[int, int, int]:
    """Resolve dashboard color names or RGB tuples to brand colors."""
    if value is None:
        return WHITE
    if isinstance(value, str):
        key = value.strip().lower()
        if key in {"pink", "brand_pink", "accent", "#ff0a72"}:
            return PINK
        if key in {"white", "#ffffff"}:
            return WHITE
        raise ValueError(
            f"Unsupported headline color {value!r}. Use 'white', 'pink', "
            "WHITE, PINK, or an RGB tuple."
        )
    if isinstance(value, (tuple, list)) and len(value) == 3:
        rgb = tuple(int(channel) for channel in value)
        if any(channel < 0 or channel > 255 for channel in rgb):
            raise ValueError(f"Headline RGB color is out of range: {value!r}")
        return rgb
    raise ValueError(f"Unsupported headline color value: {value!r}")


def get_last_headline_diagnostics(as_dict: bool = True):
    """Return diagnostics from the most recent draw_headline() call.

    Returns None before the first headline render. Set ``as_dict=False`` to
    receive the HeadlineDiagnostics dataclass instance.
    """
    if _LAST_HEADLINE_DIAGNOSTICS is None:
        return None
    return _LAST_HEADLINE_DIAGNOSTICS.to_dict() if as_dict else _LAST_HEADLINE_DIAGNOSTICS


def get_headline_qa_notes(clear: bool = False) -> List[str]:
    """Return accumulated headline QA notes; optionally clear the queue."""
    notes = list(_HEADLINE_QA_NOTES)
    if clear:
        _HEADLINE_QA_NOTES.clear()
    return notes


def clear_headline_diagnostics() -> None:
    """Clear the most recent diagnostics and accumulated QA notes."""
    global _LAST_HEADLINE_DIAGNOSTICS
    _LAST_HEADLINE_DIAGNOSTICS = None
    _HEADLINE_QA_NOTES.clear()
...
def draw_headline(draw, lines, colors, y0, sr):
    """Render a manually line-broken editorial headline via Engine v2.

    Public signature and return value are preserved for all existing templates.

    Parameters
    ----------
    lines:
        List of editorial lines. Embedded ``\n`` characters are also honored.
        The engine never rewrites or word-wraps headline copy.
    colors:
        Parallel list containing WHITE/PINK RGB tuples or the strings
        ``"white"`` and ``"pink"``. Missing entries default to white.
    y0:
        Top of the headline region.
    sr:
        ``(minimum_size, maximum_size)``. Studio 3.8 supports maxima up to
        180px and uses the largest size that fits the full block.

    Returns
    -------
    int
        Bottom y-coordinate of the rendered headline block, suitable for
        passing directly to draw_divider().
    """
    global _LAST_HEADLINE_DIAGNOSTICS

    if not lines:
        raise ValueError("draw_headline() requires at least one line")
    if not isinstance(sr, (tuple, list)) or len(sr) != 2:
        raise ValueError("draw_headline() sr must be a (minimum, maximum) pair")

    resolved_colors = [_resolve_headline_color(color) for color in (colors or [])]
    region_bottom = min(FOOTER_SAFE, int(y0) + HEAD_MAX_H)

    bottom_y, diagnostics = _render_headline_v2(
        draw,
        lines=lines,
        colors=resolved_colors,
        font_path=BARLOW,
        region=(L_MARGIN, int(y0), W - R_MARGIN, region_bottom),
        min_size=int(sr[0]),
        max_size=int(sr[1]),
        line_height_ratio=0.88,
        align="left",
        balance="optical",
        default_color=WHITE,
    )
    _LAST_HEADLINE_DIAGNOSTICS = diagnostics
    note = _headline_diagnostic_note("Headline QA", diagnostics)
    if note:
        _HEADLINE_QA_NOTES.append(note)
    return int(bottom_y)
