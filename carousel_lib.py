...
# ── INTERIOR STANDARD EXPLAINER SYSTEM (Studio 3.8 production finish) ─────
INTERIOR_HEAD_TOP = 165
INTERIOR_HEAD_MAX_H = 330
INTERIOR_HEAD_DEFAULT_RANGE = (72, 150)
INTERIOR_DIVIDER_GAP = 22
INTERIOR_BODY_GAP = 34
INTERIOR_BODY_MAX_SIZE = 46
INTERIOR_BODY_MIN_SIZE = 40
INTERIOR_BODY_LEADING = 1.30


def fit_interior_headline(draw, lines, sz_range=None):
    """Fit 1–3 manually authored interior headline lines.

    The interior system reserves enough vertical space for readable body copy.
    Short headlines are allowed to remain large; long lines shrink only as much
    as width and the interior headline zone require.
    """
    if not lines:
        raise ValueError("fit_interior_headline() requires at least one line")
    lo, hi = tuple(sz_range or INTERIOR_HEAD_DEFAULT_RANGE)
    lo, hi = int(lo), int(hi)
    for size in range(hi, lo - 1, -1):
        font = lf(BARLOW, size)
        metrics = measure_headline_block(draw, lines, font)
        if metrics["width"] <= HEAD_MAX_W and metrics["height"] <= INTERIOR_HEAD_MAX_H:
            return font, metrics, True
    font = lf(BARLOW, lo)
    return font, measure_headline_block(draw, lines, font), False


def interior_headline_y(metrics, line_count):
    """Optically place the interior headline in a stable upper field."""
    block_h = int(metrics.get("height", 0))
    if line_count <= 1:
        return INTERIOR_HEAD_TOP + 18
    if line_count == 2:
        return INTERIOR_HEAD_TOP + 8
    return INTERIOR_HEAD_TOP


def draw_interior_headline(draw, lines, colors, sz_range=None):
    """Draw an interior headline and return a measured layout report."""
    font, metrics, fit_ok = fit_interior_headline(draw, lines, sz_range)
    colors = list(colors or [])
    if len(colors) < len(lines):
        colors.extend([WHITE] * (len(lines) - len(colors)))

    top = interior_headline_y(metrics, len(lines))
    y = top
    ink_bottom = top
    for index, line in enumerate(lines):
        text = str(line)
    ...
    descender_pad = max(6, round(font.size * HEADLINE_DESCENDER_PAD_RATIO))
    glyph_bottom = ink_bottom + descender_pad
    divider_bottom = draw_divider(draw, glyph_bottom, gap=INTERIOR_DIVIDER_GAP)
    return {
        "top": top,
        "bottom": glyph_bottom,
        "divider_bottom": divider_bottom,
...
def _paragraph_segment_groups(segs):
    """Split colored body segments into paragraph groups while preserving color."""
    paragraphs = [[]]
    for text, color in (segs or []):
        chunks = str(text or "").split("\n")
        for index, chunk in enumerate(chunks):
            if chunk.strip():
                paragraphs[-1].append((chunk.strip(), color))
            if index < len(chunks) - 1:
                paragraphs.append([])
    return [p for p in paragraphs if p]


def measure_body_layout(draw, segs, font, max_w=None, paragraph_gap=None):
    """Measure wrapped body copy without drawing it."""
    max_w = max_w or (W - BODY_L - BODY_R)
    paragraph_gap = paragraph_gap if paragraph_gap is not None else max(12, round(font.size * 0.55))
    asc, desc = font.getmetrics()
    line_height = max(font.size, int((asc + desc) * INTERIOR_BODY_LEADING))
    paragraphs = _paragraph_segment_groups(segs)
    wrapped_paragraphs = [wrap_lines(draw, p, font, max_w) for p in paragraphs]
    line_count = sum(len(p) for p in wrapped_paragraphs)
    height = line_count * line_height + max(0, len(wrapped_paragraphs) - 1) * paragraph_gap
    return {
...
def fit_interior_body(draw, segs, start_y, preferred_size=46,
                      min_size=INTERIOR_BODY_MIN_SIZE):
    """Choose the largest readable body size that clears the footer safe zone."""
    preferred = max(int(preferred_size), int(min_size))
    available = max(0, FOOTER_SAFE - int(start_y))
    for size in range(preferred, int(min_size) - 1, -1):
        font = lf(BASK_REG, size)
        layout = measure_body_layout(draw, segs, font)
        if layout["height"] <= available:
            return font, layout, True
    font = lf(BASK_REG, int(min_size))
    return font, measure_body_layout(draw, segs, font), False


def draw_interior_body(draw, segs, divider_bottom, preferred_size=46,
                       min_size=INTERIOR_BODY_MIN_SIZE):
    """Draw fitted paragraph copy and return diagnostics-friendly metrics."""
    start_y = int(divider_bottom) + INTERIOR_BODY_GAP
    font, layout, fit_ok = fit_interior_body(
        draw, segs, start_y, preferred_size=preferred_size, min_size=min_size
    )
    y = start_y
    max_w = W - BODY_L - BODY_R
    for p_index, paragraph in enumerate(layout["paragraphs"]):
        for line_words in paragraph:
            x = BODY_L
            space_w = mw(draw, " ", font)
            for index, (word, color) in enumerate(line_words):
                draw.text((x, y), word, font=font, fill=color, anchor="lt")
                x += mw(draw, word, font)
                if index < len(line_words) - 1:
                    x += space_w
            y += layout["line_height"]
        if p_index < len(layout["paragraphs"]) - 1:
    ...
    return {
        ...
        "fit_ok": fit_ok,
        "available_height": max(0, FOOTER_SAFE - start_y),
        "required_height": layout["height"],
        "max_width": max_w,
    }
