...
def run_certification(repo: Path, report_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        # Studio 3.8's built-in 1×1 certification PNG is truncated and
        # Pillow cannot decode it. Generate a valid local test asset without
        # changing any Studio source file.
        dashboard.ensure_template_engine_test_asset(story_dir)
        test_asset = story_dir / "template-engine-test.png"
        try:
            from PIL import Image, ImageDraw

            image = Image.new("RGB", (1080, 1350), (28, 28, 28))
            draw = ImageDraw.Draw(image)
            draw.rectangle((0, 0, 1079, 1349), outline=(255, 10, 114), width=18)
            draw.rectangle((80, 120, 1000, 620), fill=(62, 62, 62))
            draw.rectangle((80, 690, 760, 735), fill=(255, 10, 114))
            draw.rectangle((80, 785, 960, 835), fill=(235, 235, 235))
            draw.rectangle((80, 875, 820, 925), fill=(235, 235, 235))
            image.save(test_asset, format="PNG", optimize=False)
            # Force a complete decode now so certification cannot proceed
            # with another malformed image.
            with Image.open(test_asset) as check_image:
                check_image.load()
        except Exception as exc:
            raise PhaseOneError(
                f"Could not create a valid certification image at {test_asset}: {exc}"
            ) from exc
