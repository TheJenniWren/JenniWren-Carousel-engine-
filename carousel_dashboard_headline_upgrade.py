...
def _line_value(value: Any) -> list[str]:
    """Normalize authored lines while preserving explicit line breaks.

    Supports legacy strings/lists and Editorial JSON v2 objects such as
    {"text": "CLIMATE CHANGE", "color": "pink"}.
    """
    if isinstance(value, list):
        result: list[str] = []
        for item in value:
...
def _color_value(value: Any, count: int, default: str = "white") -> list[str]:
    """Resolve one color name per authored line, with safe legacy defaults."""
    allowed = {"white", "pink"}
    colors: list[str] = []
    if isinstance(value, list):
        for item in value:
            color = str(item or default).strip().lower()
            colors.append(color if color in allowed else default)
        ...
        for item in re.split(r"[\s,]+", value.strip()):
            if item:
                ...
                colors.append(color if color in allowed else default)
    if len(colors) < count:
        colors.extend([default] * (count - len(colors)))
    return colors[:count]


def _editor_headline(slide: dict[str, Any], key: str = "headline",
                     renderer_key: str = "headline_lines") -> tuple[list[str], list[str]]:
    """Return authoritative headline lines plus parallel white/pink colors."""
    raw = slide.get(renderer_key)
    if raw is None:
        raw = slide.get(key)

    lines = _line_value(raw)
    embedded_colors: list[str] = []
    if isinstance(raw, list):
        for item in raw:
            if isinstance(item, dict) and str(item.get("text") or "").strip():
                embedded_colors.append(str(item.get("color") or "white"))
            elif not isinstance(item, dict) and str(item or "").strip():
                embedded_colors.append("white")

    explicit = slide.get("headline_colors")
    colors = _color_value(explicit if explicit is not None else embedded_colors, len(lines))
    return lines, colors
...
def adapt_slide_to_renderer(slide: dict[str, Any]) -> dict[str, Any]:
    ...
    if template == "cover_headline":
        lines, colors = _editor_headline(slide)
        deck = slide.get("deck", slide.get("body"))
        adapted.update({
            "headline_lines": lines,
            "headline_colors": colors,
        ...
        headline, headline_colors = _editor_headline(slide)
