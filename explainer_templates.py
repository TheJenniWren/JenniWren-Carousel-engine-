...
* body copy uses a fixed 26px editorial size with 31px line advance
...
EXPLAINER_BODY_SIZE = 26
EXPLAINER_BODY_MIN_SIZE = 26
EXPLAINER_BODY_LINE_ADVANCE = 31
...
def _fit_body(draw, segs, start_y: int, floor_y: int):
    for size in range(EXPLAINER_BODY_SIZE, EXPLAINER_BODY_MIN_SIZE - 1, -1):
        ...
        if start_y + report["height"] <= floor_y:
            return report
    raise ValueError(
        "body_standard copy does not fit the fixed editorial body zone at 26px. "
