...
v3.8.15
------
Production-stack Standard Explainer inline-emphasis parser fix.
...
def _prepare_body_segments(body, slide_id: str):
    """Normalize Studio body input and parse inline emphasis after import.

    Studio converts textarea body copy into a segmented list before the
    production renderer receives it. Therefore markup must be parsed inside
    every normalized segment, not only when ``body`` is still a raw string.
    """
    ...
    for seg in body_segs(body, slide_id):
        if isinstance(seg, dict):
            ...
            raw_color = seg.get("color", "white")
            base_style = BODY_BOLD if bool(seg.get("bold", False)) else BODY_REGULAR
        else:
            if len(seg) >= 2:
                text, raw_color = seg[0], seg[1]
            else:
                text, raw_color = seg[0], BODY_WHITE
            base_style = BODY_REGULAR

        base_color = BODY_PINK if raw_color == cl.PINK or str(raw_color).lower() == BODY_PINK else BODY_WHITE
        parsed = _parse_inline_body(str(text))

        if not parsed:
            continue

        for parsed_text, parsed_color, parsed_style in parsed:
            # Explicit markup wins. Otherwise preserve any color/bold styling
            # already present in the imported body segment.
            color = BODY_PINK if parsed_color == BODY_PINK else base_color
            style = BODY_BOLD if parsed_style == BODY_BOLD or base_style == BODY_BOLD else BODY_REGULAR
            prepared.append((parsed_text, color, style))
