...
def render_body_standard(slide: StorySlide, n: int, total: int, story: StoryPackage,
    ...
    # Optical vertical placement: short blocks sit slightly higher; dense blocks
    # receive a small downward correction so the full headline/body composition
    # reads balanced rather than top-heavy.
    line_count = len(lines or [])
    optical_shift = -18 if line_count <= 2 else (10 if line_count >= 4 else 0)
    y0 = max(145, cl.HEAD_Y + optical_shift)
