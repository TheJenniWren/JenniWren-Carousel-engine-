...
v3.8.8
------
Production-stack Standard Explainer calibration.
...
* body copy uses a fixed 30px editorial size with 30px line advance
...
EXPLAINER_BODY_SIZE = 30
EXPLAINER_BODY_MIN_SIZE = 30
EXPLAINER_BODY_LINE_ADVANCE = 30
EXPLAINER_BODY_MAX_W = 950
EXPLAINER_BODY_GAP = 20
...
def _fit_body(draw, segs, start_y: int, floor_y: int):
    for size in range(EXPLAINER_BODY_SIZE, EXPLAINER_BODY_MIN_SIZE - 1, -1):
        ...
        if start_y + report["height"] <= floor_y:
            return report
    raise ValueError(
        "body_standard copy does not fit the fixed editorial body zone at 30px. "
