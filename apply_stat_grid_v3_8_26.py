...
BACKUP = ROOT / "carousel_lib.py.before_stat_grid_v3_8_26"

NEW_DRAW_STAT_GRID = '''def draw_stat_grid(draw, items, y0, cols=2, cell_h=285):
    """Stat Grid v3.8.26: shift the grid slightly right while preserving the right edge."""
    items = items or []
    if not items:
        return y0
    ...
    GRID_X_SHIFT = 28
    GRID_LEFT = L_MARGIN + GRID_X_SHIFT
...
def main() -> int:
    ...
    print("Stat Grid v3.8.26 applied successfully.")
    print("Shifted the grid right while keeping the right edge stable.")
