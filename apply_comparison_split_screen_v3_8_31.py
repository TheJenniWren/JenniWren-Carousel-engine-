...
BACKUP = ROOT / "comparison_templates.py.before_comparison_split_screen_v3_8_31"

NEW_FUNCTION = r'''def render_call_block(slide: StorySlide, n: int, total: int, story: StoryPackage,
                      defaults: TemplateDefaults):
    """Comparison — Split Screen v3.8.31.
    ...
    # --- Headings ---------------------------------------------------------
    HEADLINE_Y_ADJUST = -12
    heading_y = 174 + HEADLINE_Y_ADJUST
    y = heading_y
    ...
    # --- Rich body wrapping ----------------------------------------------
    BODY_TOP_GAP = 70
    body_y = divider_y + BODY_TOP_GAP
    body_max_h = cl.FOOTER_SAFE - body_y - 22

    BODY_SIZE = 43
    BODY_LINE_ADVANCE = 51

    def fit_rich(tokens, max_w, max_h, start=BODY_SIZE, minimum=32):
        size = start
        while size >= minimum:
            ...
            if current:
                lines.append(current)
            asc, desc = font.getmetrics()
            line_h = BODY_LINE_ADVANCE
            if line_h * len(lines) <= max_h:
                return font, lines, line_h
            size -= 1
        font = cl.lf(cl.BASK_REG, minimum)
        return font, lines, BODY_LINE_ADVANCE
...
def main() -> int:
    ...
    print("Comparison — Split Screen v3.8.31 applied successfully.")
    print("Refined body size and leading, lowered the body start, and raised the headline area.")
    print("Kept the split, VS badge, headline scale, divider, footer, and arrow unchanged.")
