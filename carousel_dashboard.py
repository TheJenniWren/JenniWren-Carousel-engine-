...
def render_preview_payload(
    payload: dict[str, Any],
    slide_index: int,
) -> tuple[int, str, str, str, dict[str, Any], list[dict[str, Any]]]:
    """Trace the complete production preview path end to end."""
    ...
    try:
        ...
        _trace_step(trace, "Adapter complete", False, str(exc))
        return 1, _trace_text(trace), "", str(time.time_ns()), {}, trace
    ...
    try:
        ...
        _trace_step(trace, "Preview JSON written", False, str(exc))
        return 1, _trace_text(trace), "", str(time.time_ns()), production_payload, trace
    ...
    try:
        ...
        _trace_step(trace, "Stale preview cleared", False, str(exc))
    ...
    if exit_code != 0:
        ...
        if renderer_log:
            combined += "\n\nRenderer output:\n" + renderer_log
        return exit_code, combined, output_slug, str(time.time_ns()), production_payload, trace
    ...
    if not images:
        _trace_step(
            ...
            "PNG created",
            False,
            f"No PNG found under {preview_output}",
        )
        return 1, _trace_text(trace), output_slug, str(time.time_ns()), production_payload, trace

    image_path = images[0]
    if not image_path.exists() or image_path.stat().st_size <= 0:
        _trace_step(trace, "PNG created", False, f"Invalid PNG: {image_path}")
        return 1, _trace_text(trace), output_slug, str(time.time_ns()), production_payload, trace
    ...
    _trace_step(
        ...
        "PNG created",
        True,
        f"{image_path} ({image_path.stat().st_size:,} bytes)",
    )
    _trace_step(
        trace,
        "Browser image route ready",
        True,
        f"/image/{output_slug}/{image_path.name}",
    )

    return 0, _trace_text(trace), output_slug, str(time.time_ns()), production_payload, trace
...
<title>JenniWren Studio 3.3.1</title>
<style>
...
.trace-panel{margin-top:10px;border:1px solid var(--line);border-radius:10px;background:#0f0f0f;padding:8px}.trace-panel summary{cursor:pointer;font-weight:800}.trace-panel pre{white-space:pre-wrap;word-break:break-word;color:#ddd;font-size:12px;line-height:1.45;margin:10px 0 0}</style>
</head>
<body>
<main>
<header><h1>JenniWren Studio 3.3.1</h1><p>Production Render Trace • every preview step reports exactly what happened.</p></header>
...
class StudioHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/preview":
            ...
            try:
                exit_code, log, output_slug, render_token, production_payload, trace = render_preview_payload(payload, slide_index)
...
def main() -> int:
    ...
    print(f"JenniWren Studio 3.3.1 running on port {PORT}")
