...
def render_preview_payload(
    ...
    _trace_step(
        trace,
        "Production bridge started",
        True,
        "render_carousel.render_carousel(story_folder, output_folder)",
    )
    exit_code, renderer_log = render_story_bridge(
        story_dir,
        preview_output,
    )
    ...
    if exit_code != 0:
        _trace_step(trace, "Production bridge finished", False, f"Exit code {exit_code}")
    ...
    _trace_step(trace, "Production bridge finished", True, "Exit code 0")
...
<title>JenniWren Studio 3.5.1</title>
<style>
...
<body>
<main>
<header><h1>JenniWren Studio 3.5.1</h1><p>Production Bridge • adapted editor JSON is rendered by the production engine directly.</p></header>
...
<div class="first-cover-box real-story-box">
  <div>
    <strong>Production Bridge</strong>
    <div class="first-cover-copy">Save the adapted active slide and call the production renderer directly.</div>
  </div>
  <button type="button" id="render-current-slide">Render Through Production Bridge</button>
</div>
<div class="actions">
...
<section class="execution-answer" id="execution-answer">
  ...
  <div class="execution-title" id="execution-title">Ready to call the production renderer directly.</div>
  <div class="execution-reason" id="execution-reason">Complete the active slide, then tap Render Through Production Bridge.</div>
...
async function refreshLivePreview(){
  ...
  try{
    ...
    renderExecutionAnswer({
      status:"waiting",
      title:"Production bridge is running",
      reason:"Studio is saving renderer-native JSON and calling render_carousel.render_carousel directly.",
      next_action:"Wait for the renderer result."
    });
    setPreviewStatus("Saving JSON and calling production bridge…","busy");
    ...
    setPreviewStatus(`✓ Production bridge • ${now}`,"good");
    updateThumbnailStrip();
...
def main() -> int:
    ...
    print(f"JenniWren Studio 3.5.1 running on port {PORT}")
