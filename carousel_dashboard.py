...
<title>JenniWren Studio 3.2.1</title>
<style>
...
<body>
<main>
<header><h1>JenniWren Studio 3.2.1</h1><p>Preview Stabilization • validation can never return an undefined report.</p></header>
...
function adaptSlideToRenderer(slide){return {...slide};}
...
function validateData(data, selectedIndex=null){
  ...
  if(!data||typeof data!=="object"||Array.isArray(data)){
    ...
    return normalizeValidationReport(report,selectedIndex||0);
  }
...
function selectSlide(index,{scroll=false,render=true}={}){
  ...
  const report=normalizeValidationReport(
    validateData(buildPayload(),selectedPreviewIndex),
...
async function refreshLivePreview(){
  ...
  const rawReport=validateData(data,selectedPreviewIndex);
  const report=normalizeValidationReport(rawReport,selectedPreviewIndex);
  renderValidation(report,selectedPreviewIndex);

  const activeSlideReport=report.slides[selectedPreviewIndex]||{
    missing:[],
    template:"unknown"
  };
  const previewMissing=[
    ...
    ...report.source,
    ...report.errors,
    ...activeSlideReport.missing
  ];

  if(previewMissing.length){
    showPreview("");
    setPreviewStatus(
      `Slide ${selectedPreviewIndex+1} needs: ${previewMissing.join(", ")}`,
      "bad"
    );
    return;
  }

  setPreviewStatus("Rendering with production renderer…","busy");
...
def main() -> int:
    ...
    print(f"JenniWren Studio 3.2.1 running on port {PORT}")
