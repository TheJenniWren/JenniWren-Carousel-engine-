...
def renderer_validation_report(
    ...
    """
    Adapt editor state once, then validate the exact renderer payload used by
    preview, autosave, save, and export.
    """
    ...
    selected_index = max(
        0,
        min(int(selected_index), max(0, len(slides) - 1)),
    )

    story_missing: list[str] = []
    source_missing: list[str] = []
    general_errors: list[str] = []

    if not str(production_payload.get("story") or "").strip():
        story_missing.append("Story title")
    if not str(production_payload.get("source") or "").strip():
        source_missing.append("Primary source")
    if not slides:
        general_errors.append("At least one slide")

    slide_reports: list[dict[str, Any]] = []
    for index, slide in enumerate(slides):
        missing: list[str] = []
        template = (
            str(slide.get("template") or "unknown")
            if isinstance(slide, dict)
            else "unknown"
        )

        if not isinstance(slide, dict):
            missing.append("Valid slide data")
        else:
            required = TEMPLATE_SCHEMA_REGISTRY.get(template, {}).get("required", [])
            for key in required:
                value = slide.get(key)
                if value is None:
            ...
            if template == "timeline":
                for item_index, entry in enumerate(slide.get("timeline_entries") or []):
                    if not isinstance(entry, dict):
                        missing.append(f"Event {item_index + 1}")
                        continue
                    if not str(entry.get("date") or "").strip():
                        missing.append(f"Event {item_index + 1} date")
                    if not str(entry.get("text") or "").strip():
                        missing.append(f"Event {item_index + 1} description")
    ...
    selected_slide = (
        ...
        if slide_reports
        else {"index": 0, "template": "unknown", "missing": []}
    )
...
def render_preview_payload(
    ...
    """Render the active slide with the same production renderer used for export."""
    production_payload, _ = prepare_preview_payload(payload, slide_index)
    ...
    return exit_code, log, output_slug, production_payload
...
<title>JenniWren Studio 3.2.2</title>
<style>
...
<body>
<main>
<header><h1>JenniWren Studio 3.2.2</h1><p>Unified State • editor, validation, preview, autosave, and export use one pipeline.</p></header>
...
async function refreshLivePreview(){
  ...
  try{
    ...
    setPreviewStatus("Rendering adapted production payload…","busy");
    ...
    setPreviewStatus(`✓ Production PNG up to date • ${now}`,"good");
    updateThumbnailStrip();
...
class StudioHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/preview":
            ...
            try:
                exit_code, log, output_slug, production_payload = render_preview_payload(payload, slide_index)
...
def main() -> int:
    ...
    print(f"JenniWren Studio 3.2.2 running on port {PORT}")
