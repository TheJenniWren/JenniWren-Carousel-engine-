...
def run_renderer(folder_slug: str) -> tuple[int, str]:
    """Run the production renderer and preserve complete diagnostic output."""
    story_dir = safe_child(STORIES_DIR, folder_slug)
    command = [sys.executable, str(RENDERER), str(story_dir)]
    try:
        ...
        return (
            ...
            "\n".join(
                [
                    ...
                    f"Render timed out after 300 seconds.",
                    str(exc.stdout or ""),
...
def first_production_cover_payload() -> dict[str, Any]:
    """Known-good editorial cover used to prove the complete production path."""
    return {
        "story": "JenniWren Studio Production Test",
        "source": "JenniWren Studio internal production test",
        "slides": [
            {
                "template": "cover_headline",
                "label": "THE JENNI WREN",
                "headline": "THE ENGINE\nFINALLY WORKS",
                "deck": "This cover was generated by the production renderer.",
                "citation": "JenniWren Studio",
            }
        ],
    }
...
def prepare_preview_payload(
    ...
    """Adapt one editorial slide into the production-renderer contract."""
    ...
    return production_payload, index
...
def render_preview_payload(
    ...
    """Trace every renderer stage and return actionable diagnostics."""
    trace: list[dict[str, Any]] = []
    diagnostics: list[dict[str, str]] = []
    environment = _renderer_environment_diagnostics()

    _trace_step(trace, "Renderer file check", bool(environment["renderer_exists"]), environment["renderer"])
    _trace_step(
        trace,
    ...
    try:
        production_payload, index = prepare_preview_payload(payload, slide_index)
        _trace_step(trace, "Adapter complete", True, f"Slide {index + 1} adapted")
    except Exception as exc:
        diagnostic = _diagnostic_from_exception(exc)
        _trace_step(trace, "Adapter complete", False, diagnostic["message"])
        diagnostics.append({
        ...
        return 1, _trace_text(trace), "", str(time.time_ns()), {}, trace, diagnostics, environment, verdict
    ...
    try:
        ...
        _trace_step(trace, "Preview JSON written", True, str(carousel_path))
        _trace_step(trace, "Renderer JSON validated", True, f"{len(production_payload.get('slides', []))} slide(s)")
    except Exception as exc:
        diagnostic = _diagnostic_from_exception(exc)
        _trace_step(trace, "Preview JSON written", False, diagnostic["message"])
        diagnostics.append({
            "kind": diagnostic["type"],
            "detail": diagnostic["message"],
            "traceback": diagnostic["traceback"],
        ...
        return 1, _trace_text(trace), "", str(time.time_ns()), production_payload, trace, diagnostics, environment, verdict
    ...
    try:
        ...
        if preview_output.exists():
            for png in preview_output.glob("*.png"):
                png.unlink()
                removed += 1
        _trace_step(trace, "Stale preview cleared", True, f"{removed} PNG(s) removed")
    except Exception as exc:
        diagnostic = _diagnostic_from_exception(exc)
        _trace_step(trace, "Stale preview cleared", False, diagnostic["message"])
        diagnostics.append({
            "kind": diagnostic["type"],
            "detail": diagnostic["message"],
            "traceback": diagnostic["traceback"],
    ...
    _trace_step(trace, "Production renderer started", True, str(RENDERER))
    exit_code, renderer_log = run_renderer(PREVIEW_FOLDER)
    ...
    if exit_code != 0:
        ...
        return exit_code, combined, output_slug, str(time.time_ns()), production_payload, trace, diagnostics, environment, verdict

    _trace_step(trace, "Production renderer finished", True, "Exit code 0")

    images = image_files(output_slug)
    if not images:
        detail = f"No PNG found under {preview_output}"
        _trace_step(trace, "PNG created", False, detail)
        ...
        return 1, _trace_text(trace), output_slug, str(time.time_ns()), production_payload, trace, diagnostics, environment, verdict, verdict

    image_path = images[0]
    if not image_path.exists() or image_path.stat().st_size <= 0:
        detail = f"Invalid PNG: {image_path}"
        _trace_step(trace, "PNG created", False, detail)
        diagnostics.append({"kind": "Invalid output", "detail": detail})
        return 1, _trace_text(trace), output_slug, str(time.time_ns()), production_payload, trace, diagnostics, environment

    _trace_step(trace, "PNG created", True, f"{image_path} ({image_path.stat().st_size:,} bytes)")
    _trace_step(trace, "Browser image route ready", True, f"/image/{output_slug}/{image_path.name}")
    ...
    return 0, _trace_text(trace), output_slug, str(time.time_ns()), production_payload, trace, diagnostics, environment, verdict
...
<title>JenniWren Studio 3.4</title>
<style>
...
<div class="first-cover-box">
  <div>
    <strong>First Production PNG</strong>
    <div class="first-cover-copy">Render a known-good JenniWren cover through the real adapter and production renderer.</div>
  </div>
  <button type="button" id="render-first-cover">Render Test Cover</button>
</div>
<div class="actions">
  <button type="button" id="refresh-preview">Refresh Current Slide</button>
...
<section class="execution-answer" id="execution-answer">
  ...
  <div class="execution-title" id="execution-title">Ready to render the first production cover.</div>
  <div class="execution-reason" id="execution-reason">Tap Render Test Cover to prove the complete production path.</div>
  ...
  try{
    ...
    if(report.missing.length){
      ...
      renderExecutionAnswer({
        status:"waiting",
        title:"Renderer did not run",
        reason:`Required editorial fields are missing: ${report.missing.join(", ")}.`,
        next_action:"Complete the missing fields. Studio will then run the production renderer."
      });
      renderRawRendererOutput("Renderer was not started because validation did not pass.");
      setPreviewStatus(
        `Slide ${selectedPreviewIndex+1} needs: ${report.missing.join(", ")}`,
        "bad"
      );
      return;
    }
    ...
    renderExecutionAnswer({
      status:"waiting",
      title:"Renderer is running",
      reason:"Studio is adapting the slide, launching the production renderer, and checking for a PNG.",
      next_action:"Wait for the renderer result."
    });
    setPreviewStatus("Adapting, validating, and rendering…","busy");
    ...
    const response=await fetch("/preview",{
      ...
      body:JSON.stringify({
        payload:buildPayload(),
        slide_index:selectedPreviewIndex
      })
    });
    const result=await response.json();
    if(requestId!==previewRequestId)return;
    ...
    if(!response.ok||!result.ok){
      ...
      renderExecutionAnswer(result.verdict||{
        ...
        reason:result.error||"The renderer did not create a PNG.",
        next_action:"Open the trace and full renderer output below."
      });
      ...
      setPreviewStatus(
        `Slide ${selectedPreviewIndex+1} failed: ${result.error||"Renderer error"}`,
        "bad"
      );
      return;
    }

    rendererState=result.production_payload;
    renderExecutionAnswer(result.verdict||{
      status:"success",
      title:"PNG created successfully",
      reason:"The production renderer returned a valid PNG.",
      next_action:"Review the preview or open the production PNG."
    });
    renderProductionTrace(result.trace||[],result.log||"");
    renderRawRendererOutput(result.log||"");
    renderDiagnostics(result.diagnostics||[],result.environment||{});
    ...
    showPreview(result.image,result.render_token||"");
    const now=new Date().toLocaleTimeString([],{
      hour:"numeric",minute:"2-digit",second:"2-digit"
    });
    setPreviewStatus(`✓ Production renderer • ${now}`,"good");
    updateThumbnailStrip();
...
async function renderFirstProductionCover(){
  const button=document.getElementById("render-first-cover");
  if(button)button.disabled=true;

  renderExecutionAnswer({
    status:"waiting",
    title:"Rendering first production cover",
    reason:"Studio is sending a known-good editorial cover through the adapter and production renderer.",
    next_action:"Wait for the production PNG."
  });
  setPreviewStatus("Rendering first production cover…","busy");

  try{
    const response=await fetch("/first-production-cover",{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:"{}"
    });
    ...
    renderExecutionAnswer(result.verdict||{
      ...
      title:result.ok?"PNG created successfully":"Production cover failed",
      reason:result.ok
        ?"The real renderer created the first production cover."
        :(result.error||"The renderer did not create a PNG."),
      next_action:result.ok
        ?"Review the preview and open the PNG."
        :"Open the trace and full renderer output below."
    });

    if(!response.ok||!result.ok){
      showPreview("");
      setPreviewStatus(result.error||"First production cover failed","bad");
      return;
    }

    if(result.editorial_payload){
      populateFromData(result.editorial_payload);
      setEditorState(result.editorial_payload);
    ...
    setPreviewStatus(`✓ First production PNG • ${now}`,"good");
  }catch(error){
    showPreview("");
    renderExecutionAnswer({
      status:"failure",
      title:"First production cover failed",
      reason:error.message,
      next_action:"Open the renderer diagnostics and terminal output."
    });
    setPreviewStatus(`First production cover failed: ${error.message}`,"bad");
  }finally{
    if(button)button.disabled=false;
  }
}

document.getElementById("render-first-cover").addEventListener("click",renderFirstProductionCover);
...
class StudioHandler(BaseHTTPRequestHandler):
    server_version = "JenniWrenStudio/3.0"
    ...
    def do_POST(self) -> None:
        if self.path == "/first-production-cover":
            payload = first_production_cover_payload()
            (
                exit_code,
                log,
                output_slug,
                ...
                environment,
                verdict,
            ) = render_preview_payload(payload, 0)

            if exit_code != 0:
                self.send_json(
                    {
                        "ok": False,
                        "error": "The first production cover did not render.",
                        "verdict": verdict,
                        "trace": trace,
                        "diagnostics": diagnostics,
            ...
            if not images:
                self.send_json(
                    {
                        "ok": False,
                        "error": "Renderer exited successfully but no PNG was found.",
                        "verdict": verdict,
                        "trace": trace,
                        "diagnostics": diagnostics,
                        "environment": environment,
        ...
        if self.path == "/preview":
            ...
            try:
                exit_code, log, output_slug, render_token, production_payload, trace, diagnostics, environment, verdict = render_preview_payload(payload, slide_index)
...
def main() -> int:
    ...
    print(f"JenniWren Studio 3.4 running on port {PORT}")
