...
def _preview_placeholder_slide(slide: dict[str, Any]) -> dict[str, Any]:
    """Return a renderer-valid copy of one slide using visible placeholders."""
    item = dict(slide)
    template = str(item.get("template") or "body_standard")
    item["template"] = template
    item.setdefault("label", "SLIDE PREVIEW")
    ...
    placeholders: dict[str, dict[str, Any]] = {
        ...
        "photo_headline": {
            "image": "",
            "headline_lines": ["PHOTO STORY HEADLINE"],
            "headline_colors": ["white"],
            "citation": "SOURCE",
        },
        "stat_callout": {
        ...
        "stat_grid": {
            "headline_lines": ["KEY NUMBERS"],
            "stat_items": [
                {"stat_text": "10", "stat_label": "FIRST STAT"},
                {"stat_text": "20", "stat_label": "SECOND STAT"},
            ],
            "citation": "SOURCE",
        },
        "timeline": {
            ...
            "headline_colors": ["white"],
            "timeline_entries": [
                {"date": "DATE", "text": "Timeline event"},
            ],
            "citation": "SOURCE",
        },
        "call_block": {
            "headline_lines": ["THE POINT"],
            "call_text": [{"text": "Highlighted statement goes here."}],
            "body": [{"text": "Supporting context."}],
            "citation": "SOURCE",
        },
        "document_card": {
            "headline_lines": ["DOCUMENT EVIDENCE"],
            "headline_colors": ["white"],
            "doc_lines": ["DOCUMENT EXCERPT"],
            "doc_highlight": "HIGHLIGHT",
            "doc_annotation": [{"text": "Annotation or context."}],
            "citation": "SOURCE",
        },
        "body_standard": {
            "headline_lines": ["HEADLINE HERE"],
            "headline_colors": ["white"],
            "body": [{"text": "Body copy appears here."}],
            "citation": "SOURCE",
        },
        "sources_slide": {
    ...
    for key, value in placeholders.get(template, {}).items():
        if not _has_content(item.get(key)):
            item[key] = value

    # A photo template may still fail if the renderer requires a real image.
    # Leave the filename untouched when supplied.
    return item
...
def render_preview_payload(
    ...
    """Render only the selected slide in a temporary preview story."""
    slides = payload.get("slides")
    if not isinstance(slides, list) or not slides:
        raise ValueError("At least one slide is required for preview.")

    index = max(0, min(int(slide_index), len(slides) - 1))
    selected = slides[index] if isinstance(slides[index], dict) else {}

    preview_payload = {
        "story": str(payload.get("story") or "Studio Preview"),
        "source": str(payload.get("source") or "SOURCE"),
        "slides": [_preview_placeholder_slide(selected)],
    }
...
<title>JenniWren Studio v2.4</title>
<style>
...
function updateThumbnailStrip(){
  ...
  strip.innerHTML=Array.from({length:count},(_,i)=>{
    ...
    return `<button type="button" class="preview-thumb ${i===selectedPreviewIndex?"active":""}" data-preview-index="${i}">
...
function populateFromData(data){
  ...
  (normalized.slides.length?normalized.slides:[{template:"cover_headline",label:"THE JENNI WREN",headline_lines:["HEADLINE HERE"],headline_colors:["white"],body:[{text:"Optional deck or supporting context."}],citation:"SOURCE"}]).forEach(addSlide);
  syncPayload();
}
...
class StudioHandler(BaseHTTPRequestHandler):
    server_version = "JenniWrenStudio/2.4"
    ...
    def do_POST(self) -> None:
        if self.path == "/preview":
            ...
            image = (
                f"/image/{urllib.parse.quote(output_slug)}/{urllib.parse.quote(images[0].name)}"
                if images else ""
            )
            self.send_json({"ok": True, "image": image, "log": log})
            return
...
def main() -> int:
    ...
    print(f"JenniWren Studio v2.4 running on port {PORT}")
