...
TEMPLATE_LABELS = {key: value["label"] for key, value in TEMPLATE_SCHEMA_REGISTRY.items()}
...
def adapt_slide_to_renderer(slide: dict[str, Any]) -> dict[str, Any]:
    """Normalize Studio state to the exact production renderer contract."""
    ...
    for field in {"id", "label", *schema["required"], *schema["optional"]}:
        if field in slide:
            adapted[field] = slide[field]

    for field in ("headline_lines", "quote_lines", "doc_lines"):
        if field in adapted:
            adapted[field] = _line_value(adapted[field])
    for field in ("body",):
        if field in adapted:
            adapted[field] = _body_value(adapted[field])

    if "headline_lines" in adapted:
        adapted["headline_colors"] = _line_value(adapted.get("headline_colors")) or [
            "white" for _ in adapted["headline_lines"]
        ]
    if "quote_lines" in adapted:
        adapted["quote_colors"] = _line_value(adapted.get("quote_colors")) or [
            "white" for _ in adapted["quote_lines"]
        ]

    if template == "call_block":
        value = adapted.get("call_text")
        if isinstance(value, list):
            value = " ".join(str(item.get("text") if isinstance(item, dict) else item) for item in value)
        adapted["call_text"] = str(value or "").strip()
    ...
    if template == "stat_grid":
        ...
        for item in items:
            if isinstance(item, dict):
                ...
                label = str(item.get("stat_label") or item.get("label") or "").strip()
            elif isinstance(item, (list, tuple)) and len(item) >= 2:
                stat, label = str(item[0]).strip(), str(item[1]).strip()
            else:
                continue
            if stat or label:
                normalized.append([stat, label])
        adapted["stat_items"] = normalized
    ...
    if template == "timeline":
        ...
        adapted["timeline_entries"] = [
            ...
            for item in entries
            if isinstance(item, dict)
            and (str(item.get("date") or item.get("label") or "").strip()
                 or str(item.get("text") or item.get("event") or "").strip())
        ]

    if template == "document_card":
        adapted["doc_highlight"] = _index_value(adapted.get("doc_highlight"))
        adapted["doc_annotation"] = bool(adapted.get("doc_annotation", True))

    if template == "sources_slide":
        adapted["citations"] = [
            str(item.get("citation") or item.get("text") or "").strip()
            if isinstance(item, dict) else str(item).strip()
            for item in (adapted.get("citations") or [])
            if (str(item.get("citation") or item.get("text") or "").strip()
                if isinstance(item, dict) else str(item).strip())
        ]
    return adapted
...
<title>JenniWren Studio 3.0</title>
<style>
...
<body>
<main>
<header><h1>JenniWren Studio 3.0</h1><p>Production-renderer registry, exact adapters, and real PNG previews.</p></header>
...
for(const [template,schema] of Object.entries(SCHEMA_REGISTRY)){
  if(!Array.isArray(schema.widgets)){
    showAppError("Template registry entry failed",`${template}: missing widget definitions`);
    continue;
  }
  schemas[template]=schema.widgets.map(widget=>[widget.renderer,widget.label,widget.type]);
...
function readSlide(slide,index){const template=slide.querySelector("[data-template]").value;const item={template,label:slide.querySelector("[data-label]").value.trim()||`SLIDE ${index+1}`};for(const [key,,type] of schemas[template]||[]){const repeater=slide.querySelector(`[data-repeater="${key}"]`);if(repeater){item[key]=readRepeater(repeater);continue}const input=slide.querySelector(`[data-key="${key}"]`);if(!input)continue;if(type==="boolean"){item[key]=input.checked;continue}const value=input.value.trim();if(type==="lines")item[key]=value.split(/\n+/).map(v=>v.trim()).filter(Boolean);else if(type==="body")item[key]=value?[{text:value}]:[];else if(type==="indices")item[key]=value;else if(value)item[key]=value}if(["cover_headline","photo_headline","timeline","document_card","body_standard"].includes(template))item.headline_colors=["white"];if(template==="quote_lead")item.quote_colors=["white"];return item}
...
def main() -> int:
    ...
    print(f"JenniWren Studio 3.0 running on port {PORT}")
