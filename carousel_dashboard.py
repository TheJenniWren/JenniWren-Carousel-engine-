...
EDITORIAL_SCHEMAS: dict[str, dict[str, Any]] = {
    ...
    "stat_callout": {
        ...
        "fields": [
            ...
            {"name": "context", "label": "Context / headline", "type": "headline"},
            {"name": "citation", "label": "Citation", "type": "text"},
        ],
    },
    "stat_grid": {
    ...
    "call_block": {
        ...
        "required": ["statement"],
        "fields": [
            {"name": "headline", "label": "Optional headline", "type": "headline"},
...
def _editor_headline(slide: dict[str, Any], primary: str = "headline", fallback: str = "headline_lines") -> tuple[list[str], list[str]]:
    """Return manual headline lines and per-line colors, preserving v3.7 input.

    Accepted editor forms:
      * a newline-delimited string
      * a list of strings
      * a list of {"text": ..., "color": "white"|"pink"} objects

    Renderer-native headline_lines/headline_colors remain supported.
    """
    raw = slide.get(primary)
    if raw in (None, "", []):
        raw = slide.get(fallback, [])

    inherited_colors = slide.get("headline_colors", []) or []
    lines: list[str] = []
    colors: list[str] = []
    ...
    if isinstance(raw, str):
        ...
        for index, item in enumerate(raw):
            if isinstance(item, dict):
                value = str(item.get("text") or item.get("line") or "").strip()
                color = str(item.get("color") or "white").strip().lower()
                if value:
                    lines.append(value)
                    colors.append("pink" if color == "pink" else "white")
            else:
    ...
    if not lines:
        lines = _editor_lines(slide, primary, fallback)

    if len(colors) != len(lines):
        colors = []
        for index in range(len(lines)):
            candidate = str(inherited_colors[index]).lower() if index < len(inherited_colors) else "white"
            colors.append("pink" if candidate == "pink" else "white")
...
def adapt_slide_to_renderer(slide: dict[str, Any]) -> dict[str, Any]:
    ...
    if template == "cover_headline":
        lines, line_colors = _editor_headline(slide, "headline", "headline_lines")
        deck = slide.get("deck", slide.get("body"))
        adapted.update({
            "headline_lines": lines,
            "headline_colors": line_colors,
        ...
        context, context_colors = _editor_headline(slide, "context", "headline_lines")
        adapted.update({
            ...
            "headline_lines": context,
            "headline_colors": context_colors,
        })

    elif template == "stat_grid":
        headline, headline_colors = _editor_headline(slide, "headline", "headline_lines")
...
def _headline_editor_value(slide: dict[str, Any], lines_key: str = "headline_lines", colors_key: str = "headline_colors") -> list[dict[str, str]]:
    lines = slide.get(lines_key, []) or []
    colors = slide.get(colors_key, []) or []
    ...
    for index, line in enumerate(lines):
        ...
        if not value:
            continue
        color = str(colors[index]).lower() if index < len(colors) else "white"
        result.append({"text": value, "color": "pink" if color == "pink" else "white"})
    return result
...
def renderer_slide_to_editor(slide: dict[str, Any]) -> dict[str, Any]:
    ...
    if template == "cover_headline":
        ...
        editor["context"] = _headline_editor_value(slide)
    elif template == "stat_grid":
        editor["headline"] = _headline_editor_value(slide)
...
function normalizeHeadlineRows(value){
  if(Array.isArray(value)&&value.length){
    return value.map((item,index)=>typeof item==="object"
      ? {text:String(item.text||item.line||""),color:String(item.color||"white").toLowerCase()==="pink"?"pink":"white"}
      : {text:String(item||""),color:"white"});
  }
  if(typeof value==="string"&&value.trim()){
    return value.split(/\r?\n/).filter(line=>line.trim()).map(line=>({text:line.trim(),color:"white"}));
  }
...
function genericEditorHTML(data={}){
  return `
    ...
    ${fieldHTML("headline","headline","Headline",data.headline||data.headline_lines||"")}
    <label>Body</label>
...
function normalizeHeadlinePayload(value, inheritedColors=[]){
  const rows=[];
  if(typeof value==="string"){
    value.split(/\r?\n/).forEach((line,index)=>{
      const text=cleanPayloadString(line);
      if(text)rows.push({
        text,
        color:String(inheritedColors[index]||"white").toLowerCase()==="pink"?"pink":"white"
      });
    });
  }else if(Array.isArray(value)){
    value.forEach((item,index)=>{
      const text=cleanPayloadString(
        item&&typeof item==="object" ? (item.text||item.line) : item
      );
      if(!text)return;
      const requested=item&&typeof item==="object"
        ? item.color
        : inheritedColors[index];
      rows.push({
        text,
        color:String(requested||"white").toLowerCase()==="pink"?"pink":"white"
      });
    });
  }
  return rows;
...
function buildProductionSlide(slideElement,index){
  const raw=readSlide(slideElement,index);
  const template=cleanPayloadString(raw.template)||"body_standard";
  const result={
    ...raw,
    template,
    label:cleanPayloadString(raw.label)||`SLIDE ${index+1}`
  };

  const schemaFields=schemas[template]||[];
  const headlineField=schemaFields.find(([, , type])=>type==="headline")?.[0];
  if(headlineField){
    const rows=normalizeHeadlinePayload(
      raw[headlineField],
      Array.isArray(raw.headline_colors)?raw.headline_colors:[]
    );
    result[headlineField]=rows;
    result.headline_lines=rows.map(row=>row.text);
    result.headline_colors=rows.map(row=>row.color);

    // Legacy compatibility for validators and renderers that still read headline.
    if(headlineField==="headline"){
      result.headline=rows.map(row=>row.text).join("\n");
    }
  }

  // Normalize common text fields while preserving repeaters and booleans.
  ["deck","body","citation","quote","attribution","image","statement",
   "call_text","statistic","statistic_label","stat_text","stat_label"].forEach(key=>{
    if(key in result && typeof result[key]!=="object"){
      result[key]=cleanPayloadString(result[key]);
    }
  });
...
function readEditorStateFromDOM(){
  const title=cleanPayloadString(document.getElementById("story_title")?.value);
  const source=cleanPayloadString(document.getElementById("source")?.value);
  const folder=cleanPayloadString(document.getElementById("folder_slug")?.value);
  return {
    story:title,
    title,
    folder_slug:folder,
    source,
    slides:[...slidesRoot.querySelectorAll("[data-slide]")]
      .map((slide,index)=>buildProductionSlide(slide,index))
  };
}

function normalizeEditorStory(nextState){
  const storyValue=nextState?.story;
  if(storyValue&&typeof storyValue==="object"){
    return cleanPayloadString(storyValue.title||storyValue.name||nextState?.title);
  }
  return cleanPayloadString(storyValue||nextState?.title);
}

function setEditorState(nextState){
  editorState={
    story:normalizeEditorStory(nextState),
    title:normalizeEditorStory(nextState),
    folder_slug:cleanPayloadString(
      nextState?.folder_slug ||
      (nextState?.story&&typeof nextState.story==="object"?nextState.story.folder_slug:"")
    ),
    source:cleanPayloadString(nextState?.source),
...
function buildPayload(){
  // The DOM is the source of truth. Never render stale editorState data.
  return cloneJSON(syncEditorStateFromDOM());
}
...
slidesRoot.addEventListener("click",event=>{
  ...
  if(button.matches("[data-add-row]")){
    ...
    if(type==="headline")repeater.insertAdjacentHTML("beforeend",headlineRowHTML({text:"",color:"white"}));
...
function slideSummaryText(slide,index){
  ...
  const text=Array.isArray(lead)?lead.map(item=>typeof item==="object"?item.text||"":item).join(" "):String(lead);
...
function populateFromData(data){
  const storyObject=data?.story&&typeof data.story==="object" ? data.story : {};
  const storyTitle=cleanPayloadString(
    storyObject.title || storyObject.name ||
    (typeof data?.story==="string"?data.story:"") ||
    data?.title
  );
  const folderSlug=cleanPayloadString(
    data?.folder_slug || storyObject.folder_slug || data?.slug
  );
  const sourceValue=cleanPayloadString(data?.source || storyObject.source);
  const incomingSlides=Array.isArray(data?.slides)?data.slides:[];

  document.getElementById("story_title").value=storyTitle;
  document.getElementById("source").value=sourceValue;
  if(folderSlug&&document.getElementById("folder_slug")){
    document.getElementById("folder_slug").value=folderSlug;
  }
  slidesRoot.innerHTML="";

  const incoming=incomingSlides.length
    ? incomingSlides
    : [{
        template:"cover_headline",
        label:"THE JENNI WREN",
  ...
  incoming.forEach((slideData,index)=>{
    try{
      const normalizedSlide={...slideData};
      if(
        !normalizedSlide.headline &&
        Array.isArray(normalizedSlide.headline_lines)
      ){
        const colors=Array.isArray(normalizedSlide.headline_colors)
          ? normalizedSlide.headline_colors : [];
        normalizedSlide.headline=normalizedSlide.headline_lines.map((line,lineIndex)=>({
          text:cleanPayloadString(
            line&&typeof line==="object" ? (line.text||line.line) : line
          ),
          color:String(
            line&&typeof line==="object" ? line.color : colors[lineIndex]||"white"
          ).toLowerCase()==="pink"?"pink":"white"
        })).filter(item=>item.text);
      }
      addSlide(normalizedSlide);
    }catch(error){
      showAppError(
