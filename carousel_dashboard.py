...
def render_preview_payload(
    ...
    try:
        ...
        return 1, _trace_text(trace), "", str(time.time_ns()), {}, trace, diagnostics, environment
    ...
    try:
        ...
        return 1, _trace_text(trace), "", str(time.time_ns()), production_payload, trace, diagnostics, environment
    ...
    if exit_code != 0:
        ...
        if renderer_log:
            combined += "\n\nRenderer output:\n" + renderer_log
        return exit_code, combined, output_slug, str(time.time_ns()), production_payload, trace, diagnostics, environment
    ...
    return 0, _trace_text(trace), output_slug, str(time.time_ns()), production_payload, trace, diagnostics, environment
...
<title>JenniWren Studio 3.3.2</title>
<style>
...
class StudioHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/preview":
            ...
            try:
                exit_code, log, output_slug, render_token, production_payload, trace, diagnostics, environment = render_preview_payload(payload, slide_index)
...
def main() -> int:
    ...
    print(f"JenniWren Studio 3.3.2 running on port {PORT}")
