...
def render_preview_payload(payload: dict[str, Any]) -> tuple[int, str, str]:
    """Render a temporary preview carousel without overwriting the active story."""
...
<title>JenniWren Studio v2.3.1</title>
<style>
...
<div id="preview-status" class="preview-status"><span class="status-dot"></span>Waiting for complete slide data</div>
<div id="live-frame" class="live-frame"><div class="live-empty">Complete the required fields for a slide. Studio will render it automatically.</div></div>
<div id="preview-strip" class="preview-strip"></div>
<div class="actions"><button type="button" id="refresh-preview">Refresh Preview</button></div>
</aside>
</div>
...
function showPreview(index=selectedPreviewIndex) {
  const frame = document.getElementById("live-frame");
  const strip = document.getElementById("preview-strip");

  if (!previewUrls.length) {
    frame.innerHTML = '<div class="live-empty">No preview image is available yet.</div>';
    strip.innerHTML = "";
    return;
  }

  selectedPreviewIndex = Math.max(0, Math.min(index, previewUrls.length - 1));
  const cacheKey = Date.now();
  frame.innerHTML = `<img src="${previewUrls[selectedPreviewIndex]}?t=${cacheKey}" alt="Live slide preview">`;
  strip.innerHTML = previewUrls.map((url, i) => `
    <button type="button" class="preview-thumb ${i===selectedPreviewIndex?"active":""}" data-preview-index="${i}">
      <img src="${url}?t=${cacheKey}" alt="Slide ${i+1}">
    </button>`).join("");
}
...
async function refreshLivePreview() {
  ...
  if (errors.length) {
    setPreviewStatus(errors[0], "bad");
    return;
  }

  const requestId = ++previewRequestId;
  setPreviewStatus("Rendering preview…", "busy");
  ...
  try {
    ...
    if (!response.ok || !result.ok) {
      setPreviewStatus(result.error || "Preview failed.", "bad");
      return;
    }

    previewUrls = result.images || [];
    showPreview();
    setPreviewStatus(
      `Preview updated • ${previewUrls.length} slide${previewUrls.length===1?"":"s"}`,
      "good"
    );
  } catch (error) {
    if (requestId !== previewRequestId) return;
    setPreviewStatus(`Preview failed: ${error.message}`, "bad");
  }
}
...
function schedulePreview() {
  ...
  previewTimer = setTimeout(refreshLivePreview, 1200);
}
...
document.getElementById("studio-form").addEventListener("input", schedulePreview);
document.getElementById("studio-form").addEventListener("change", schedulePreview);
...
populateFromData(initial);schedulePreview();
</script>
</main>
</body>
</html>'''
...
class StudioHandler(BaseHTTPRequestHandler):
    server_version = "JenniWrenStudio/2.3.1"
    ...
    def do_POST(self) -> None:
        if self.path == "/preview":
            ...
            if errors:
                self.send_json(
                    {"ok": False, "error": errors[0], "errors": errors},
                    HTTPStatus.BAD_REQUEST,
                )
                return

            exit_code, log, output_slug = render_preview_payload(payload)
            if exit_code != 0:
                self.send_json(
                    {"ok": False, "error": "Renderer preview failed.", "log": log},
                    HTTPStatus.BAD_REQUEST,
                )
                return

            images = [
                f"/image/{urllib.parse.quote(output_slug)}/{urllib.parse.quote(path.name)}"
                for path in image_files(output_slug)
            ]
            self.send_json({"ok": True, "images": images, "log": log})
            return
...
def main() -> int:
    ...
    print(f"JenniWren Studio v2.3.1 running on port {PORT}")
